...
use WpAlgolia\Index\IndexReplicaSettings;
...
class IndexSettingsTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        new IndexSettings(array('replicas' => true));
    }

    public function testCanRetrieveReplicaSettings()
    {
        $replicaSettings1 = array(
            'customRanking' => array('asc(name)'),
        );
        $replicaSettings2 = array(
            'customRanking' => array('desc(name)'),
        );
        $settings = new IndexSettings(array(
            'replicas' => array(
                'products_asc' => $replicaSettings1,
                'products_desc' => $replicaSettings2,
            ),
        ));
        ...
        $this->assertInstanceOf('WpAlgolia\Index\IndexReplicaSettings', $replicaSettings[0]);
        ...
        $this->assertInstanceOf('WpAlgolia\Index\IndexReplicaSettings', $replicaSettings[1]);
    ...
    {
        $replicaSettings = new IndexReplicaSettings('test');
        $settings = new IndexSettings(array(
            'replicas' => array(
                $replicaSettings,
            ),
